...
<body>
  <div class="container">
    <div id="formArea">
      ...
      <p style="font-size:14px;color:#6586a7;margin:0 0 20px;background:#f0f5ff;border-radius:12px;padding:12px;">
        Hesap: <strong>{{ $email }}</strong>
      </p>

      <div class="field">
        <label>Adın Soyadın</label>
        <input type="text" id="name" placeholder="Adın Soyadın" />
      </div>
    ...
    <div class="success-msg" id="successArea">
      ...
      <p style="color:#6586a7;margin:0;">Panele yönlendiriliyorsunuz...</p>
    </div>
  </div>
  ...
  <script>
    async function setPassword() {
      ...
      if (!name) { errEl.textContent = 'Adını ve soyadını gir.'; errEl.style.display = 'block'; return; }
      ...
      try {
        const res = await fetch('{{ url("/ogretmen/sifre-belirle/" . $token) }}', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
          body: JSON.stringify({ name, password, password_confirmation: confirm })
        });
